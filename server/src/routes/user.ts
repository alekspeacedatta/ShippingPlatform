...
router.patch('/edit/:id', authenticateJWT, async (req: Request, res: Response) => {
  try {
    ...
    const selectedUser = await UserModel.findByIdAndUpdate(id, updateData, { new: true });
...
router.post('/create-request', async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ message: 'parcel request created successfuly', parcelRequest: parcelRequest });
  } catch (error) {
    console.error(error);
