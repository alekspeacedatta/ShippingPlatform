    ...
    <>
      <div className="flex min-h-screen flex-col items-center justify-center">
        <div className="min-w-5xl mx-auto flex flex-col gap-5">
            ...
            {parcel.timeline.length === 0 ? (
              ...
              <div className="space-y-6">
                {parcel.timeline.map((t, idx) => {
                  ...
                  const when =
                    // @ts-ignore
                    typeof t.at === 'string' || t.at instanceof Date
                      ? new Date(t.at).toLocaleString()
                      : String(t.at);

                  return (
                    <div
                      key={`${t.at}-${t.status}-${idx}`}
          ...
          <div>
            <h2 className="text-2xl font-semibold">Total price <span className='font-bold text-green-500'>{parcel.priceEstimate}$</span></h2>
          </div>
        </div>
      </div>
    </>
