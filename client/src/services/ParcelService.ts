...
export class Parcelservice {
  ...
  static async getParcelRequests(id: string) {
    const res = await fetch(`${BASE_URL}/api/parcel/get-requests?id=${encodeURIComponent(id)}`, {
      method: 'GET',
    });
  ...
  static async getParcelRequest(parcelId: string) {
    const res = await fetch(`${BASE_URL}/api/parcel/get?parcelId=${encodeURIComponent(parcelId)}`, {
      method: 'GET',
    });
